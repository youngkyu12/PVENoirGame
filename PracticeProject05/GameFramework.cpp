...
void GameFramework_Build::BuildObjects()
{
	...
	m_pScene = make_unique<Scene_Build>(m_buildRes);
	if (m_pScene) m_pScene->BuildObjects(d3dDevice.Get());

	m_buildRes->m_GameTimer.Reset();
}

void GameFramework_Build::ReleaseObjects()
{
}
	...
	: m_buildRes(buildRes)
{
	...
	m_pScene = make_unique<Scene_Render>(m_buildRes);
...
void GameFramework_Render::OnResizeBackBuffers()
{
	...
	ComPtr<IDXGIFactory4>& dxgiFactory = m_buildRes->m_pdxgiFactory;
	ComPtr<IDXGISwapChain3>& dxgiSwapChain = m_buildRes->m_pdxgiSwapChain;
	ComPtr<ID3D12Resource>& d3dSwapChainBackBuffers = m_buildRes->m_ppd3dSwapChainBackBuffers[m_buildRes->m_nSwapChainBuffers];
	ComPtr<ID3D12Resource>& d3dDepthStencilBuffer = m_buildRes->m_pd3dDepthStencilBuffer;
...
{
	if (m_pScene) 
		m_pScene->AnimateObjects(m_buildRes->m_GameTimer.GetTimeElapsed());
}
...
void GameFramework_Render::FrameAdvance()
{
	...
	// 렌더링 코드는 여기에 추가될 것이다.
	if (m_pScene) 
		m_pScene->Render(d3dCommandList.Get());
	...
	d3dCommandQueue->ExecuteCommandLists(_countof(ppd3dCommandLists), ppd3dCommandLists);
...
void GameFramework_Render::MoveToNextFrame()
{
	...
	UINT64 nFenceValue = ++m_buildRes->m_nFenceValues[m_buildRes->m_nSwapChainBufferIndex];
