...
namespace
{
	constexpr float kWorldMinX = -600.0f;
	constexpr float kWorldMaxX = 600.0f;
	constexpr float kWorldMinZ = -200.0f;
	constexpr float kWorldMaxZ = 1000.0f;
	...
	void WorldToGL(float worldX, float worldZ, float& outX, float& outY)
	{
		const float tx = Clamp01((worldX - kWorldMinX) / (kWorldMaxX - kWorldMinX));
		const float tz = Clamp01((worldZ - kWorldMinZ) / (kWorldMaxZ - kWorldMinZ));
		outX = tx * 2.0f - 1.0f;
		outY = 1.0f - tz * 2.0f;
	}

	void DrawPointList(const std::vector<DrawPoint>& points, float r, float g, float b, float size)
	{
		glColor3f(r, g, b);
		glPointSize(size);
		glBegin(GL_POINTS);
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glPrint("DummyClient World View");
