...
namespace
{
		...
		size_t& total)
	{
		...
		sprintf_s(
			buf,
			"[GameSceneMemory] %-48s count=%8zu cap=%8zu elem=%4zu bytes=%12zu (%.3f MiB)\n",
			name,
			count,
			capacity,
			elemSize,
	...
	template <typename T>
	static size_t VectorStorageBytes(const std::vector<T>& v)
	{
		return sizeof(T) * v.capacity();
	}

	static size_t VectorStorageBytes(const std::vector<bool>& v)
	{
		return ( v.capacity() + 7 ) / 8;
	}

	template <typename T>
	static void DumpVectorMemory(
		const char* name,
		...
		size_t& total)
	{
		DumpMemoryLine(
			...
			v.capacity(),
			sizeof(T),
			VectorStorageBytes(v),
			total
		);
	}

	static void DumpVectorBoolMemory(
		const char* name,
	...
	template <typename T>
	static size_t ApproxUnorderedSetBytes(const std::unordered_set<T>& s)
	{
		// 정확한 allocator/node overhead는 STL 구현마다 다르다.
		// bucket 배열 + 노드당 대략적인 포인터 overhead를 더한 추정치.
		return
			s.bucket_count() * sizeof(void*) +
			s.size() * ( sizeof(T) + sizeof(void*) * 3 );
	}

	template <typename T>
	static void DumpUnorderedSetMemory(
		const char* name,
		const std::unordered_set<T>& s,
		size_t& total)
	{
		const size_t bytes = ApproxUnorderedSetBytes(s);

		char buf[512];
		sprintf_s(
			buf,
			"[GameSceneMemory] %-48s size=%8zu buckets=%8zu approxBytes=%12zu (%.3f MiB)\n",
			name,
			s.size(),
			s.bucket_count(),
			bytes,
	...
	template <typename K, typename V>
	static void DumpUnorderedMapMemory(
		const char* name,
		const std::unordered_map<K, V>& m,
		size_t& total)
	{
		const size_t bytes = ApproxUnorderedMapBytes(m);
		...
		size_t& total)
	{
		...
		sprintf_s(
			buf,
			"[GameSceneMemory] %-48s bytes=%12zu (%.3f MiB)\n",
			name,
			bytes,
			BytesToMiB(bytes)
		);
...
void CGameScene::DumpBuildMemoryReport() const
{
#if defined(_DEBUG) || defined(DEBUG)
	OutputDebugStringA("\n");
	OutputDebugStringA("============================================================\n");
	OutputDebugStringA("[GameSceneMemory] Build memory report begin\n");
	OutputDebugStringA("============================================================\n");

	{
			...
			sizeof(pmc)) )
		{
			...
			sprintf_s(
				buf,
				"[GameSceneMemory][Process] WorkingSet=%zu bytes (%.3f MiB), PrivateUsage=%zu bytes (%.3f MiB)\n",
				static_cast< size_t >( pmc.WorkingSetSize ),
				BytesToMiB(static_cast< size_t >( pmc.WorkingSetSize )),
				static_cast< size_t >( pmc.PrivateUsage ),
				BytesToMiB(static_cast< size_t >( pmc.PrivateUsage ))
			);
			OutputDebugStringA(buf);
		}
	}

	{
		...
		sprintf_s(
			buf,
			"[GameSceneMemory][Counts] staticObjects=%zu/%u skinnedObjects=%zu/%u colliderObjects=%zu/%u "
			"staticGroups=%zu skinnedGroups=%zu staticLod=%zu skinnedLod=%zu staticOcc=%zu skinnedOcc=%zu "
			"skinnedBonePaletteCapacity=%u\n",
			m_staticObjects.size(),
			m_staticBatch.capacity,
			m_skinnedObjects.size(),
			m_skinnedBatch.capacity,
			m_colliderObjects.size(),
			m_colliderBatch.capacity,
			m_staticInstanceGroups.size(),
			m_skinnedInstanceGroups.size(),
			m_staticWorldLodEntries.size(),
			m_skinnedWorldLodEntries.size(),
			m_staticOcclusionEntries.size(),
			m_skinnedOcclusionEntries.size(),
			m_skinnedBonePaletteCapacity
		);
		OutputDebugStringA(buf);
	}

	size_t cpuVectorTotal = 0;
	size_t cpuHashTotal = 0;
	size_t gpuKnownTotal = 0;

	OutputDebugStringA("\n[GameSceneMemory] ---- CPU vector storage ----\n");

	DumpVectorMemory("m_staticObjects", m_staticObjects, cpuVectorTotal);
	DumpVectorMemory("m_skinnedObjects", m_skinnedObjects, cpuVectorTotal);
	DumpVectorMemory("m_colliderObjects", m_colliderObjects, cpuVectorTotal);

	DumpVectorMemory("m_itemBillboards", m_itemBillboards, cpuVectorTotal);
	DumpVectorMemory("m_muzzleFlashes", m_muzzleFlashes, cpuVectorTotal);
	DumpVectorMemory("m_bossPoisonProjectiles", m_bossPoisonProjectiles, cpuVectorTotal);
	DumpVectorMemory("m_swordTrails", m_swordTrails, cpuVectorTotal);
	DumpVectorMemory("m_monsterSwordTrails", m_monsterSwordTrails, cpuVectorTotal);

	DumpVectorMemory("m_ghoulRefs", m_ghoulRefs, cpuVectorTotal);
	DumpVectorMemory("m_swordManRefs", m_swordManRefs, cpuVectorTotal);
	DumpVectorMemory("m_bowManRefs", m_bowManRefs, cpuVectorTotal);
	DumpVectorMemory("m_MutantRefs", m_MutantRefs, cpuVectorTotal);
	DumpVectorMemory("m_bossRefs", m_bossRefs, cpuVectorTotal);
	DumpVectorMemory("m_helmetRefs", m_helmetRefs, cpuVectorTotal);

	DumpVectorMemory("m_PlayerSwordRefs", m_PlayerSwordRefs, cpuVectorTotal);
	DumpVectorMemory("m_PlayerBowRefs", m_PlayerBowRefs, cpuVectorTotal);
	DumpVectorMemory("m_PlayerAxeRefs", m_PlayerAxeRefs, cpuVectorTotal);
	DumpVectorMemory("m_PlayerGunRefs", m_PlayerGunRefs, cpuVectorTotal);
	DumpVectorMemory("m_EnemySwordRefs", m_EnemySwordRefs, cpuVectorTotal);
	DumpVectorMemory("m_EnemyBowRefs", m_EnemyBowRefs, cpuVectorTotal);
	DumpVectorMemory("m_EnemySpawnRefs", m_EnemySpawnRefs, cpuVectorTotal);
	DumpVectorMemory("m_enemySpawnPoolEntries", m_enemySpawnPoolEntries, cpuVectorTotal);

	DumpVectorMemory("m_attachmentBinds", m_attachmentBinds, cpuVectorTotal);
	DumpVectorMemory("m_arrowRefs", m_arrowRefs, cpuVectorTotal);
	DumpVectorMemory("m_bulletRefs", m_bulletRefs, cpuVectorTotal);
	DumpVectorMemory("m_preparedBowmanArrows", m_preparedBowmanArrows, cpuVectorTotal);

	DumpVectorBoolMemory("m_prevEnemyBowReleasePhase", m_prevEnemyBowReleasePhase, cpuVectorTotal);
	DumpVectorBoolMemory("m_prevGhoulAttackPhase", m_prevGhoulAttackPhase, cpuVectorTotal);
	DumpVectorBoolMemory("m_prevSwordManAttackPhase", m_prevSwordManAttackPhase, cpuVectorTotal);
	DumpVectorBoolMemory("m_prevMutantAttackPhase", m_prevMutantAttackPhase, cpuVectorTotal);
	DumpVectorBoolMemory("m_prevBowManSfxLoadPhase", m_prevBowManSfxLoadPhase, cpuVectorTotal);

	DumpVectorMemory("m_monsterFootstepSfxStates", m_monsterFootstepSfxStates, cpuVectorTotal);
	DumpVectorMemory("m_pendingMonsterSfxList", m_pendingMonsterSfxList, cpuVectorTotal);
	DumpVectorMemory("m_activeMonsterSfxList", m_activeMonsterSfxList, cpuVectorTotal);

	DumpVectorMemory("m_staticInstanceGroups", m_staticInstanceGroups, cpuVectorTotal);
	DumpVectorMemory("m_staticRenderObjectCache", m_staticRenderObjectCache, cpuVectorTotal);
	DumpVectorMemory("m_staticGameplayTickObjects", m_staticGameplayTickObjects, cpuVectorTotal);
	DumpVectorMemory("m_staticWorldLodEntries", m_staticWorldLodEntries, cpuVectorTotal);
	DumpVectorMemory("m_staticOcclusionEntries", m_staticOcclusionEntries, cpuVectorTotal);
	DumpVectorMemory("m_staticWorldLodEntryIndexByObjectIndex", m_staticWorldLodEntryIndexByObjectIndex, cpuVectorTotal);

	DumpVectorMemory("m_staticDistanceCullFlags", m_staticDistanceCullFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticOcclusionCullFlags", m_staticOcclusionCullFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticTreeGridCullFlags", m_staticTreeGridCullFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticDynamicWorldMatrixFlags", m_staticDynamicWorldMatrixFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticShadowCasterFlags", m_staticShadowCasterFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticTreeObjectIndices", m_staticTreeObjectIndices, cpuVectorTotal);
	DumpVectorMemory("m_staticShadowOcclusionEntryIndices", m_staticShadowOcclusionEntryIndices, cpuVectorTotal);
	DumpVectorMemory("m_staticCollisionMegaGridMasks", m_staticCollisionMegaGridMasks, cpuVectorTotal);

	DumpVectorMemory("m_staticOcclusionQuerySampleCounts", m_staticOcclusionQuerySampleCounts, cpuVectorTotal);
	DumpVectorMemory("m_staticOcclusionLastFrameIssuedFlags", m_staticOcclusionLastFrameIssuedFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticOcclusionCurrentFrameIssuedFlags", m_staticOcclusionCurrentFrameIssuedFlags, cpuVectorTotal);
	DumpVectorMemory("m_staticOcclusionZeroSampleFrameCounts", m_staticOcclusionZeroSampleFrameCounts, cpuVectorTotal);

	DumpVectorMemory("m_skinnedInstanceGroups", m_skinnedInstanceGroups, cpuVectorTotal);
	DumpVectorMemory("m_skinnedComponentCache", m_skinnedComponentCache, cpuVectorTotal);
	DumpVectorMemory("m_skinnedWorldLodEntries", m_skinnedWorldLodEntries, cpuVectorTotal);
	DumpVectorMemory("m_skinnedDistanceCullFlags", m_skinnedDistanceCullFlags, cpuVectorTotal);
	DumpVectorMemory("m_skinnedOcclusionEntries", m_skinnedOcclusionEntries, cpuVectorTotal);
	DumpVectorMemory("m_skinnedOcclusionCullFlags", m_skinnedOcclusionCullFlags, cpuVectorTotal);
	DumpVectorMemory("m_skinnedShadowOcclusionEntryIndices", m_skinnedShadowOcclusionEntryIndices, cpuVectorTotal);
	DumpVectorMemory("m_skinnedOcclusionQuerySampleCounts", m_skinnedOcclusionQuerySampleCounts, cpuVectorTotal);
	DumpVectorMemory("m_skinnedOcclusionLastFrameIssuedFlags", m_skinnedOcclusionLastFrameIssuedFlags, cpuVectorTotal);
	DumpVectorMemory("m_skinnedOcclusionCurrentFrameIssuedFlags", m_skinnedOcclusionCurrentFrameIssuedFlags, cpuVectorTotal);
	DumpVectorMemory("m_skinnedOcclusionZeroSampleFrameCounts", m_skinnedOcclusionZeroSampleFrameCounts, cpuVectorTotal);

	DumpVectorMemory("m_skinnedBonePaletteBaseByObject", m_skinnedBonePaletteBaseByObject, cpuVectorTotal);
	DumpVectorMemory("m_skinnedBonePaletteCountByObject", m_skinnedBonePaletteCountByObject, cpuVectorTotal);

	DumpVectorMemory("m_monsterGridTrackers", m_monsterGridTrackers, cpuVectorTotal);
	DumpVectorMemory("m_arrowGridTrackers", m_arrowGridTrackers, cpuVectorTotal);
	DumpVectorMemory("m_bulletGridTrackers", m_bulletGridTrackers, cpuVectorTotal);
	DumpVectorMemory("m_skinnedMonsterMegaGridNumbers", m_skinnedMonsterMegaGridNumbers, cpuVectorTotal);

#ifndef USING_NETWORK
	DumpVectorMemory("m_monsterSpawnEntries", m_monsterSpawnEntries, cpuVectorTotal);
	DumpVectorMemory("m_towerDoorPortals", m_towerDoorPortals, cpuVectorTotal);
	DumpVectorMemory("m_castleDoorPortals", m_castleDoorPortals, cpuVectorTotal);
#endif

	OutputDebugStringA("\n[GameSceneMemory] ---- CPU hash storage, approximate ----\n");

	DumpUnorderedMapMemory("m_mutantKeyTriggerMegaByObject", m_mutantKeyTriggerMegaByObject, cpuHashTotal);
	DumpUnorderedSetMemory("m_deadMonsters", m_deadMonsters, cpuHashTotal);
	DumpUnorderedMapMemory("m_bossMeleeSlashCastStates", m_bossMeleeSlashCastStates, cpuHashTotal);
	DumpUnorderedMapMemory("m_bossPoisonSpellCastStates", m_bossPoisonSpellCastStates, cpuHashTotal);
	DumpUnorderedMapMemory("m_bossStageBossPositionStates", m_bossStageBossPositionStates, cpuHashTotal);

#ifdef USING_NETWORK
	DumpUnorderedMapMemory("m_prevPlayerNetworkStateCode", m_prevPlayerNetworkStateCode, cpuHashTotal);
#endif

	OutputDebugStringA("\n[GameSceneMemory] ---- Known D3D12 buffer bytes from GameScene capacities ----\n");

	DumpRawMemoryLine(
		"m_staticBatch.cbGameObjects total",
		static_cast< size_t >( m_staticBatch.cbElementBytes ) *
		static_cast< size_t >( m_staticBatch.capacity ) *
		static_cast< size_t >( kFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_skinnedBatch.cbGameObjects total",
		static_cast< size_t >( m_skinnedBatch.cbElementBytes ) *
		static_cast< size_t >( m_skinnedBatch.capacity ) *
		static_cast< size_t >( kFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_colliderBatch.cbGameObjects total",
		static_cast< size_t >( m_colliderBatch.cbElementBytes ) *
		static_cast< size_t >( m_colliderBatch.capacity ) *
		static_cast< size_t >( kFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dcbLights total",
		static_cast< size_t >( m_nLightsCBElementBytes ) *
		static_cast< size_t >( kFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dcbMaterials total",
		static_cast< size_t >( m_nMaterialsCBElementBytes ) *
		static_cast< size_t >( kFrameResourceCount ),
		gpuKnownTotal
	);

	{
		const UINT kStaticInstancePassCount = 2;
		DumpRawMemoryLine(
			"m_pd3dStaticInstanceBuffer total",
			static_cast< size_t >( sizeof(StaticInstanceVertex) ) *
			static_cast< size_t >( m_staticInstanceBufferCapacity ) *
			static_cast< size_t >( kStaticInstancePassCount ) *
			static_cast< size_t >( kSceneBatchFrameResourceCount ),
			gpuKnownTotal
		);
	}

	{
		const UINT kSkinnedInstancePassCount = 2;
		DumpRawMemoryLine(
			"m_pd3dSkinnedInstanceBuffer total",
			static_cast< size_t >( sizeof(SkinnedInstanceVertex) ) *
			static_cast< size_t >( m_skinnedInstanceBufferCapacity ) *
			static_cast< size_t >( kSkinnedInstancePassCount ) *
			static_cast< size_t >( kSceneBatchFrameResourceCount ),
			gpuKnownTotal
		);
	}
	...
	DumpRawMemoryLine(
		"m_pd3dSkinnedBonePaletteBuffer total",
		static_cast< size_t >( sizeof(XMFLOAT4X4) ) *
		static_cast< size_t >( m_skinnedBonePaletteCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dItemBillboardInstanceBuffer total",
		static_cast< size_t >( sizeof(ItemBillboardInstanceVertex) ) *
		static_cast< size_t >( m_itemBillboardInstanceBufferCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dTransparentItemBillboardInstanceBuffer total",
		static_cast< size_t >( sizeof(ItemBillboardInstanceVertex) ) *
		static_cast< size_t >( m_transparentItemBillboardInstanceBufferCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dMuzzleFlashInstanceBuffer total",
		static_cast< size_t >( sizeof(MuzzleFlashInstanceVertex) ) *
		static_cast< size_t >( m_muzzleFlashInstanceBufferCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dBossPoisonProjectileInstanceBuffer total",
		static_cast< size_t >( sizeof(MuzzleFlashInstanceVertex) ) *
		static_cast< size_t >( m_bossPoisonProjectileInstanceBufferCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dSwordTrailVertexBuffer total",
		static_cast< size_t >( sizeof(SwordTrailVertex) ) *
		static_cast< size_t >( m_swordTrailVertexBufferCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_pd3dMonsterSwordTrailVertexBuffer total",
		static_cast< size_t >( sizeof(MonsterSwordTrailVertex) ) *
		static_cast< size_t >( m_monsterSwordTrailVertexBufferCapacity ) *
		static_cast< size_t >( kSceneBatchFrameResourceCount ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_staticOcclusionReadbackBuffer estimated",
		static_cast< size_t >( sizeof(UINT64) ) *
		static_cast< size_t >( m_staticOcclusionQueryCapacity ),
		gpuKnownTotal
	);

	DumpRawMemoryLine(
		"m_skinnedOcclusionReadbackBuffer estimated",
		static_cast< size_t >( sizeof(UINT64) ) *
		static_cast< size_t >( m_skinnedOcclusionQueryCapacity ),
		gpuKnownTotal
	);

	{
		...
		sprintf_s(
			buf,
			"\n[GameSceneMemory][Summary] cpuVectorStorage=%zu bytes (%.3f MiB), "
			"cpuHashApprox=%zu bytes (%.3f MiB), knownGameSceneD3D12Buffers=%zu bytes (%.3f MiB)\n",
			cpuVectorTotal,
			BytesToMiB(cpuVectorTotal),
			cpuHashTotal,
			BytesToMiB(cpuHashTotal),
			gpuKnownTotal,
			BytesToMiB(gpuKnownTotal)
		);
		OutputDebugStringA(buf);
	}

	OutputDebugStringA("============================================================\n");
	OutputDebugStringA("[GameSceneMemory] Build memory report end\n");
	OutputDebugStringA("============================================================\n\n");
#endif
}