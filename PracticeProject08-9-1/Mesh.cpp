...
float CHeightMapGridMesh::OnGetHeight(int x, int z, void* pContext)
{
	...
	const int sx = std::clamp(x, 0, terrain->GetHeightMapWidth() - 1);
	const int sz = std::clamp(z, 0, terrain->GetHeightMapLength() - 1);
...
XMFLOAT4 CHeightMapGridMesh::OnGetColor(int x, int z, void* pContext)
{
	...
	float n0 = Vector3::DotProduct(terrain->SampleHeightMapNormal(x0, z0), lightDir);
	float n1 = Vector3::DotProduct(terrain->SampleHeightMapNormal(x1, z0), lightDir);
	float n2 = Vector3::DotProduct(terrain->SampleHeightMapNormal(x1, z1), lightDir);
	float n3 = Vector3::DotProduct(terrain->SampleHeightMapNormal(x0, z1), lightDir);
	float ndotl = std::clamp(( n0 + n1 + n2 + n3 ) * 0.25f, 0.0f, 1.0f);

	XMFLOAT4 litColor(0.9f, 0.8f, 0.4f, 1.0f);
	return XMFLOAT4(litColor.x * ndotl, litColor.y * ndotl, litColor.z * ndotl, 1.0f);
}