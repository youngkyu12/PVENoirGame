...
void CGameFramework::BuildSceneInternal(ESceneId id, bool resetTimer)
{
	...
	// 2) PostProcess 재생성 (Scene 전환 시 SRV 힙이 새로 만들어지므로 반드시 재빌드)
	...
	// RTV heap에서 swapchain 뒤에 붙여서 postprocess RT들을 만든다.
	D3D12_CPU_DESCRIPTOR_HANDLE d3dRtvCPUDescriptorHandle = m_pd3dRtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	...
	// Depth SRV도 새 Scene의 SRV 힙에 다시 만든다.
...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	// MenuScene이 요청을 올리면 여기서 받아서 다음 프레임에 Scene 전환
	if (scene)
	{
...
void CGameFramework::FrameAdvance()
{
	...
	// ★ MenuScene 클릭 요청이 있으면 여기서 GameScene을 lazy build 후 전환
